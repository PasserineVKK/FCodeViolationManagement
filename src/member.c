...
void removeOneMember (MemberList *members, AccountList *accounts, ViolationList *violations, const char *id, const char *actorID){
    ...
    if (mIndex != -1) {
        ...
        if (members->data[mIndex].role == 2 && members->data[actorIndex].role < 2){
            printf ("You cannot remove a BOD member.\n");
            return;
        }    
        else if (members->data[mIndex].role == 2 && members->data[actorIndex].role == 2  && checkTotalBOD (members)== 1){
            // You can only remove yourself if you are not the last BOD.
...
void addMember(MemberList* members, AccountList* accounts, const char *actorID) {
    ...
    int continueAdd = 1;
    // Let the user add more than one member.
    while (continueAdd) {
        // Input student ID
        ...
        if (searchMemberByIdInM(members, studentID) == -1) {
            ...
            if (confirm == 1) {
                ...
                strcpy(newAcc->password, studentID);  // Default password is the student ID.
                newAcc->role = role;
                newAcc->isLocked = 0;
                newAcc->failCount = 0;
            ...
            printf("Student ID already exists. Try a different ID.\n");
        }
...
void updateMember(AccountList* accounts, MemberList* members, ViolationList* violations, const char *actorID) {
    ...
    while (continueUpdate) {
        ...
        if (mIndex != -1) {
            ...
            if (members->data[mIndex].role == 2 && members->data[actorIndex].role < 2){
                printf ("You cannot update a BOD member.\n");
                continue;
            }    
