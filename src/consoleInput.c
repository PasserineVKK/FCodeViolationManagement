...
// Reads a password string (min length enforced) and stores it in `target`.
// The function does not hide input (console echo) — it's a simple helper.
void inputPassword(char* target, const char* prompt) {
    ...
    while (1) {
        ...
        if (strlen(buf) < 6) {
            uiError("Password must be at least 6 characters.\n");
            continue;
        }
        strcpy(target, buf);
        return;
