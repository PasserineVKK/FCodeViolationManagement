...
long getRemainingLockTime(Account *acc) {
    if (acc->isLocked) return -1; 
    if (acc->failCount == 0 || acc->failCount % 3 != 0) return 0;

    time_t now = time(NULL);
    double diff = difftime(now, acc->lockedFrom);
    long lockDuration = 0;

    if (acc->failCount == 3) lockDuration = 300;      // lock in 5 min
    else if (acc->failCount == 6) lockDuration = 3600; // lock in 1 hour
    else if (acc->failCount >= 9) return -1;           // Require admin to unlock
